...
function addToEncodedSignatureMap(signature) {
  // skip... already done
  if (encodedSignatureMapReverse.has(signature)) return;
  // strip leading 0x
...
export function signatureToAbi(sig, signatures = []) {
  signatures
...
export function decodeFunctionCall(data_, signatures = []) {
  // striop leading 0x
  ...
  const paramsResult = web3.eth.abi.decodeParameters(
    functionAbi.inputs,
    `0x${encodedParams}`
  );
